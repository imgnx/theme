...
default_namespace() {
    PARENT_DIR="$(realpath ../)"
    echo "$(basename $PARENT_DIR)/$(basename $PWD)"
}

theme_ai_color() {
    local dir_base="${PWD:t}"
    local dir_abs="${PWD:A}"
    local ns="${NAMESPACE:-$dir_base}"
    local items
    items=$(ls -1A | head -n "${THEME_AI_MAX_ITEMS:-8}" | tr '\n' ' ')
    local prompt="Return one hex color (#RRGGBB) that fits the vibe of directory '${dir_base}' (namespace '${ns}') at path '${dir_abs}'. Nearby items: ${items}. Respond with only the color."
    local timeout="${THEME_AI_TIMEOUT:-4}"

    if [[ -n "${THEME_AI_CMD:-}" ]]; then
	local -a ai_cmd
	ai_cmd=("${(z)THEME_AI_CMD}")
	if (( ${#ai_cmd[@]} )); then
	    local out
	    if out=$(THEME_AI_PROMPT="$prompt" THEME_AI_BASENAME="$dir_base" THEME_AI_NAMESPACE="$ns" THEME_AI_PATH="$dir_abs" THEME_AI_ITEMS="$items" "${ai_cmd[@]}" 2>/dev/null); then
		out="${out%%$'\n'*}"
		[[ -n "$out" ]] && printf '%s\n' "$out" && return 0
	    fi
	fi
    ...
    if [[ -n "${THEME_AI_HTTP_URL:-}" && -x "$(command -v curl)" ]]; then
	local body resp hex
	body=$(printf '{"prompt":"%s","basename":"%s","namespace":"%s","path":"%s","items":"%s"}' \
	    "${prompt//\"/\\\"}" "${dir_base//\"/\\\"}" "${ns//\"/\\\"}" "${dir_abs//\"/\\\"}" "${items//\"/\\\"}")
	resp=$(curl -sS --max-time "$timeout" -H 'Content-Type: application/json' -d "$body" "$THEME_AI_HTTP_URL" 2>/dev/null) || resp=""
	hex=$(printf '%s' "$resp" | sed -n 's/.*\\(#[0-9A-Fa-f]\\{6\\}\\).*/\\1/p' | head -n1)
	[[ -n "$hex" ]] && printf '%s\n' "$hex" && return 0
    fi
...
THEME_ROLL() {
    ...
    if new="$(theme_ai_color)"; then
	echo "$new"
	if is_hex "$new"; then
	    new="$(normalize_hex "$new")"
	    used_ai=1
	else
	    new=""
	fi
    else
	echo "\`new\` is not a hex."
	echo "new: $new"
    fi
    ...
    save_themefile
    if (( used_ai )); then
	print -P $'%F{green}Applied AI-suggested theme and saved to .themefile.%f'
    else
...
export NAMESPACE="${NAMESPACE:-${SESSION_NAME:-$(default_namespace)}}"
